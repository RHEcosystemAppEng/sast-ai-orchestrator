...
import com.redhat.sast.api.v1.dto.response.DatasetStorageInfoResponseDto;
...
public class DatasetStorageResource {
     ...
     *
     * @return storage provider type and information
     */
    @GET
    @Path("/info")
    public Response getStorageInfo() {
        try {
            String storageType = datasetStorageService.getStorageType();
            String storageInfo = datasetStorageService.getStorageInfo();

            return Response.ok(new DatasetStorageInfoResponseDto(storageType, storageInfo))
                    .build();
        } catch (Exception e) {
            LOGGER.error("Failed to get storage info", e);
            return Response.serverError()
                    .entity(new ErrorResponseDto("Failed to get storage info", e.getMessage()))
                    .build();
        }
    }

    /**
     * Initialize dataset storage PVCs or S3 buckets.
     *
